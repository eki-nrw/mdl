...
*/
class ProposalPlan extends AbstractPlan implements ProposalPlanInterface
{
	/**
	* @inheritdoc
	*/
	protected function matchPlanType(PlanTypeInterface $planType)
	{
		...
		if (!$planType->is('proposal'))
			throw new \InvalidArgumentException("Proposal Plan must be proposal plan type.");
	}
	...
	*/
	protected function validatePlanItem(PlanItemInterface $planItem)
	{
		...
		if (!$planItem->getPlanItemType()->is('proposal'))
			throw new \InvalidArgumentException("Proposal Plan don't accept not-proposal plan item.");
	}
}
