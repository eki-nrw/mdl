...
/**
 * Implemetation Executor as Callable Executor 
 */
class CallableExecutor implements Executor
{
    /**
	* @inheritdoc
	*/
	...
	*/
    public function execute(View $view)
    {
    	...
    	if (!isset($action['arguments']))
	       	call_user_func($action['executable']);
		else
       		call_user_func($action['executable'], $action['arguments']);
	}
}
